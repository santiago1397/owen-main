...
SCOPE: `ai_agent` runtime and the recordings pipeline are LATER tickets. Here `ai_agent` is a
stub that routes to its `default` port (falling through to the fallback), and `record` merely
drives ARI record — the WAV fetch/transcribe reuse is ticket 05's job. `dial` supports a
NUMBER target and (Ticket 13) an OPERATOR target (individual or group; via `dial_operator`).
"""
